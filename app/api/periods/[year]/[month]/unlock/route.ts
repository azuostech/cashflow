...
import { z } from 'zod';
import { isValidPeriod, unlockPeriod } from '@/lib/periods/close';
...
export async function POST(request: NextRequest, { params }: { params: { year: string; month: string } }) {
  ...
  if (session.role !== 'owner') {
    return NextResponse.json({ error: 'Apenas o owner pode desbloquear periodos' }, { status: 403 });
  }
  ...
  try {
    await unlockPeriod(session.companyId, year, month, session.userId, parsed.data.justification, request);
    return NextResponse.json({ unlocked: true, year, month });
  } catch (error) {
